...
- [ ] **AgentCore Memory observability enabled** — either Transaction Search (emits Memory OTel spans to the `aws/spans` CloudWatch Log Group) or Memory structured logging. **Both are opt-in. Without one of them enabled before the incident, there is no retrospective detection path for memory writes** (see 2.2). To enable: turn on account-wide CloudWatch **Transaction Search** (Console → CloudWatch → Application Signals → Transaction search → *Ingest spans as structured logs*), then configure per-Memory log/trace delivery — see the [AgentCore observability-configure docs](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability-configure.html).
